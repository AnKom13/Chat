...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        group_send_dict = {"type": "chat_message", "message": 'joined', "username": self.user.username,
                           "usernames": usernames, 'whom': 'room', 'whomname': '---', }
    ...
    async def receive(self, text_data):
        ...
        whomname = text_data_json["whomname"]
        usernames = cache['my'].get(f'{self.room_name}', None)
        # Send message to room group
        await self.channel_layer.group_send(
            self.room_group_name, {"type": "chat_message", "message": message, "username": username,
                                   "usernames": usernames, 'whom': whom, 'whomname': whomname, }
        )
    ...
    async def chat_message(self, event):
        ...
        if whom == 'room':
            ...
            if self.channel_name == whom:
                name_whom = usernames[whom]
                await self.send(text_data=json.dumps({"message": message, "username": username,
                                                      "usernames": usernames, 'from': username}))
            # мое. кидает сообщение себе
            whom = next(k for k, v in usernames.items() if v == username)
            if self.channel_name == whom:
                await self.send(text_data=json.dumps({"message": message, "username": username,
                                                      "usernames": usernames, 'from': whomname}))

    def add_cache(self):
        group = cache['my'].get(f'{self.room_name}', None)
        if group:
            group[self.channel_name] = self.user.username
        else:
            group = {self.channel_name: self.user.username}
        cache['my'][f'{self.room_name}'] = group
        return group
